...
//			LCD_ShowString(0,285,lcddev.width,lcddev.height,12,msg);
//		}		
		return Data;
}
...
void VS_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOE, ENABLE);	 //使能PB端口时钟
	
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 //PE2 SO
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;	 //PE5 SI
...
//		retry++;
		delay_us(50);
//		LCD_ShowString(0,250,lcddev.width,lcddev.height,12,"VS1053 test!");
	};
	delay_ms(20);	
...
//	VS_WR_Cmd(SPI_MODE,0x0820);
	u16 ret1 =	VS_RD_Reg(SPI_STATUS);
	u16 ret0 =	VS_RD_Reg(SPI_MODE);
//	VS_WR_Cmd(SPI_STATUS,0x0035);
//	u16 ret1 =	VS_RD_Reg(SPI_STATUS);
  u8 msg[64];
	sprintf((char*)msg,"ret0=%x re1=%x\n",ret0,ret1);
//	sprintf((char*)msg,"ret0=%x\n",ret0);
	LCD_ShowString(0,285,lcddev.width,lcddev.height,12,msg);
}
...
//	SPI_Write(VS_READ_COMMAND);
	VS_SPI_WriteByte(address);       	//地址
//	SPI_Write(address);
	temp=VS_SPI_ReadByte(0xff); 		//读取高字节
//	temp = SPI_Read();
	
//		u8 msg[20];
//		if (temp != 0) {
//			sprintf((char*)msg,"Data0 = %d\n",temp);
//			LCD_ShowString(0,285,lcddev.width,lcddev.height,12,msg);
//		}		
		
	temp=temp<<8;
	temp+=VS_SPI_ReadByte(0xff); 		//读取低字节
//	temp = SPI_Read();
		
		
//		u8 msg1[20];
//		if (temp != 0) {
//			sprintf((char*)msg1,"Data1 = %d\n",temp);
//			LCD_ShowString(100,285,lcddev.width,lcddev.height,12,msg1);
//		}
...
{
	VS_XDCS_SET();   //MP3_DATA_CS=0;
	VS_SPI_WriteByte(data);
	VS_XDCS=1;   //MP3_DATA_CS=1;
	VS_XCS=1;   //MP3_CMD_CS=1; 
}

void Vs1003_DATAS_Write(unsigned char* data)
{
	u8 n;
